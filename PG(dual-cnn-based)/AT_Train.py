...
from AT_cnn_Policy import PolicyNetwork, cnn
...
def main():
    ...
    # Print the shapes of each parameter
    # Load the state dictionary
    # state_dict = torch.load("target_nn.pth") 
    # for param_name, param_tensor in state_dict.items():
    #     print(f"Layer: {param_name} | Shape: {param_tensor.shape}")

    # input_dim = (84, 84)
    policy_agent = ActorCritic(obs, output_dim)
    policy_agent.net.load_state_dict(torch.load("policy_net_2000.pth", map_location=device))
    target_agent = ActorCritic(obs, output_dim)
    target_agent.net.load_state_dict(torch.load("target_net_2000.pth", map_location=device))
    # policy_agent.net.shared_net.eval() 
    ...
    for episode in range(MAX_EPISODES):
        ...
        while not done:
            ...
            with torch.no_grad():
                ...
                action = policy_agent.sample_action(state_tensor, total_reward_tensor)
            ...
            env.render()
        # add penalty for time taken to complete episode
        # end = time.time()
        # print("Time taken: ", end - start)
        # replay_buffer.
        # Save rewards for analysis
        reward_over_episodes.append(total_reward)
        with open('/reward_cnn_pg_episodes.pkl', 'wb') as f:
